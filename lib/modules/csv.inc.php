...
class sv_woocommerce_order_export_module_csv{
	...
	public function build($data){
		...
		if(isset($_POST['subscriptions']) && $_POST['subscriptions'] == 'all'){
			$this->titles				= $this->data->settings->get_subscriptions_user_settings();
			$this->globalSettings		= $this->data->settings->get_subscriptions_global_settings();
			$this->orders				= $this->data->get_subscriptions();
		}else{
			$this->titles				= $this->data->settings->get_user_settings();
			$this->globalSettings		= $this->data->settings->get_global_settings();
	...
	private function content(){
		...
		foreach($this->orders_filtered as $order_id => $fields){ // all orders
			if(isset($this->globalSettings['settings']['nested']) && $this->globalSettings['settings']['nested'] == 'data_flat'){
				$child								= $this->processChilds($order_id);
				...
				fputcsv($this->output, $fields); // new row
			}else{
				fputcsv($this->output, $fields); // new row
				$this->processChilds($order_id); // new row
			}
		}
	}
	private function processChilds($order_id){
		$out										= array();
			...
			foreach($this->childs->get_childs_by($order_id) as $item){
				foreach($item as $options){
					foreach($this->get_fields() as $field_position => $field_id){
						if(!isset($out[$field_position]) || $out[$field_position] == ''){
							@$out[$field_position]		= preg_replace("/[\r\n]+/", " ",$options[$field_id]);
						}
					}
					if(empty($this->globalSettings['settings']['nested']) || $this->globalSettings['settings']['nested'] != 'data_flat'){
						fputcsv($this->output, $out); // new row
						$out							= array();
					}
				}
			}
