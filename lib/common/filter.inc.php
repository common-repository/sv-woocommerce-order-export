...
class sv_woocommerce_order_export_filter{
	...
	public function strip_item($order_id,$item_id){
		$this->stripped_items[$order_id][$item_id]		= true;
	}
		...
		foreach($order['items'] as $item_id => $item){
			if(!$this->is_item_stripped($order['order']->get_order_number(),$item_id)){
				$field_map = array(
					...
					'item_author_name'			=> 'items_author_name',
	...
	public function data_flat($field,$order){ // we will use the data_new_row method again here, as the new row directive will be overwritten in output-module
		$this->data_new_row($field,$order);
	}
	...
	public function invoice_id($order){
		...
		if(isset($GLOBALS['woocommerce_germanized'])){
			...
			$output = get_post_meta($order['order']->get_order_number(),'_wcpdf_formatted_invoice_number',true);
	...
	public function order_date($order){
		return apply_filters(__METHOD__,$order['order']->get_date_created(),$order,$this);
	}
	public function order_status($order){
		return apply_filters(__METHOD__,$order['order']->get_status(),$order,$this);
	}
	public function payment_method($order){
		return apply_filters(__METHOD__,$order['order']->get_payment_method(),$order,$this);
	}
	public function download_permissions_granted($order){
		return apply_filters(__METHOD__,$order['order']->is_download_permitted(),$order,$this);
	}
	public function customer_id($order){
		return apply_filters(__METHOD__,$order['order']->get_user_id(),$order,$this);
	}
	public function billing_first_name($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_first_name(),$order,$this);
	}
	public function billing_last_name($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_last_name(),$order,$this);
	}
	public function billing_full_name($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_first_name().' '.$order['order']->get_billing_last_name(),$order,$this);
	}
	public function billing_company($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_company(),$order,$this);
	}
	public function billing_email($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_email(),$order,$this);
	}
	public function billing_phone($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_phone(),$order,$this);
	}
	public function billing_address_1($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_address_1(),$order,$this);
	}
	public function billing_address_2($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_address_2(),$order,$this);
	}
	public function billing_full_address($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_address_1().' '.$order['order']->get_billing_address_2(),$order,$this);
	}
	public function billing_postcode($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_postcode(),$order,$this);
	}
	public function billing_city($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_city(),$order,$this);
	}
	public function billing_country($order){
		return apply_filters(__METHOD__,$order['order']->get_billing_country(),$order,$this);
	}
	public function shipping_first_name($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_first_name(),$order,$this);
	}
	public function shipping_last_name($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_last_name(),$order,$this);
	}
	public function shipping_full_name($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_first_name().' '.$order['order']->get_shipping_last_name(),$order,$this);
	}
	public function shipping_company($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_company(),$order,$this);
	}
	public function shipping_address_1($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_address_1(),$order,$this);
	}
	public function shipping_address_2($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_address_2(),$order,$this);
	}
	public function shipping_full_address($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_address_1().' '.$order['order']->get_shipping_address_2(),$order,$this);
	}
	public function shipping_postcode($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_postcode(),$order,$this);
	}
	public function shipping_city($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_city(),$order,$this);
	}
	public function shipping_country($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_country(),$order,$this);
	}
	public function shipping_method($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_method(),$order,$this);
	}
	public function shipping_costs($order){
		return apply_filters(__METHOD__,$order['order']->get_shipping_total(),$order,$this);
	}
	public function order_comments($order){
		return apply_filters(__METHOD__,$order['order']->get_customer_note(),$order,$this);
	}
	...
	private function item_total_tax_percent($item){
		if(floatval($item['line_total']) > 0){
			...
			return apply_filters(__METHOD__,0,$item,$this);
		}
	}
	...
	private function item_author_name($item){
		return apply_filters(__METHOD__,get_the_author_meta('login',get_post_field('post_author', $item['product_id'])),$item,$this);
	}
	public function items_author_name($order){
		return apply_filters(
			__METHOD__,
			call_user_func(
				array($this,$this->get_multiple_data_handler()),
